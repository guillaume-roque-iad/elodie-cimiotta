 ...
 *  - aucun script Google n'est chargé tant que l'utilisateur n'a pas cliqué "Acceptar" ;
...
(function () {
  ...
  // qu'après un clic explicite sur "Acceptar" (ou un choix "accepté" déjà valide).
  function loadGA() {
  ...
  function initConsentBanner() {
    ...
    function showBanner() {
      ...
      b.setAttribute('aria-label', 'Consentiment de galetes');
      ...
      b.innerHTML =
        ...
        'Aquest lloc utilitza Google Analytics per mesurar l’audiència, només si ho accepteu. ' +
        'Google Analytics no es carrega fins que premeu «Acceptar». Podeu canviar d’opinió en qualsevol moment; consulteu la nostra ' +
        '<a href="/ca/mentions-legales#rgpd" style="color:#0079A3;text-decoration:underline">política de privacitat</a>.</p>' +
        '<div style="display:flex;gap:10px;flex-shrink:0">' +
        '<button type="button" id="ccRefuse" style="padding:10px 22px;border-radius:999px;border:1.5px solid #3C4A5F;background:#fff;color:#3C4A5F;font-family:inherit;font-weight:700;font-size:13px;cursor:pointer">Rebutjar</button>' +
        '<button type="button" id="ccAccept" style="padding:10px 22px;border-radius:999px;border:none;background:#00B4EC;color:#000;font-family:inherit;font-weight:700;font-style:italic;font-size:13px;cursor:pointer">Acceptar</button>' +
        '</div>';
    ...
    function ensureManageLink() {
      ...
      a.id = 'ccManage';
      a.href = '#';
      a.textContent = "Galetes";
      a.setAttribute('aria-label', 'Gestionar les preferències de galetes');
  ...
  var GENERIC_ERROR = "No s’ha pogut enviar la vostra sol·licitud. Comproveu les dades o contacteu directament amb Élodie.";
  ...
  function initSecureForm(cfg) {
    ...
    f.addEventListener('submit', function (e) {
      ...
      if (!nom || !email) {
        showFormError(errBox, 'Introduïu el vostre nom i correu electrònic.');
        return;
      }
      if (!/^[^\s@]+@[^\s@]+\.[^\s@]+$/.test(email)) {
        showFormError(errBox, 'Introduïu una adreça electrònica vàlida.');
        return;
      }

      submitting = true;
      if (btn) { btn.textContent = 'S’està enviant...'; btn.disabled = true; }
