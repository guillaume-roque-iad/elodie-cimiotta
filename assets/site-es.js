 ...
 *  - aucun script Google n'est chargé tant que l'utilisateur n'a pas cliqué "Aceptar" ;
...
(function () {
  ...
  // qu'après un clic explicite sur "Aceptar" (ou un choix "accepté" déjà valide).
  function loadGA() {
  ...
  function initConsentBanner() {
    ...
    function showBanner() {
      ...
      b.innerHTML =
        ...
        'Este sitio utiliza Google Analytics para medir la audiencia, únicamente si lo acepta. ' +
        'Google Analytics no se carga hasta que pulse «Aceptar». Puede cambiar de opinión en cualquier momento; consulte nuestra ' +
        '<a href="/es/mentions-legales#rgpd" style="color:#0079A3;text-decoration:underline">política de privacidad</a>.</p>' +
        '<div style="display:flex;gap:10px;flex-shrink:0">' +
        '<button type="button" id="ccRefuse" style="padding:10px 22px;border-radius:999px;border:1.5px solid #3C4A5F;background:#fff;color:#3C4A5F;font-family:inherit;font-weight:700;font-size:13px;cursor:pointer">Rechazar</button>' +
        '<button type="button" id="ccAccept" style="padding:10px 22px;border-radius:999px;border:none;background:#00B4EC;color:#000;font-family:inherit;font-weight:700;font-style:italic;font-size:13px;cursor:pointer">Aceptar</button>' +
        '</div>';
    ...
    function ensureManageLink() {
      ...
      a.setAttribute('aria-label', 'Gestionar las preferencias de cookies');
  ...
  var GENERIC_ERROR = "No se ha podido enviar su solicitud. Compruebe los datos o contacte directamente con Élodie.";
  ...
  function initSecureForm(cfg) {
    ...
    f.addEventListener('submit', function (e) {
      ...
      if (!nom || !email) {
        showFormError(errBox, 'Introduzca su nombre y su correo electrónico.');
        return;
      }
      if (!/^[^\s@]+@[^\s@]+\.[^\s@]+$/.test(email)) {
        showFormError(errBox, 'Introduzca una dirección de correo electrónico válida.');
        return;
      }
